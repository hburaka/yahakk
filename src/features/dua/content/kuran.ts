import type { Dua } from '../types';

/**
 * Gözden geçirildi ve onaylandı.
 *
 * Kur'an-ı Kerîm'de geçen dualar. Arapça metin kamu malı; Türkçe
 * anlamlar telifli bir mealden ALINMADI, sade biçimde kendimiz yazdık.
 *
 * Sıralama mushaf sırasına göre — konuya göre değil. Kullanıcı bir
 * duayı sûre adıyla hatırlıyor ve listede aradığı yerde bulmalı.
 */

export const KURAN_DUALARI: readonly Dua[] = [
  {
    id: 'bakara-201',
    categoryId: 'kuran',
    title: 'Dünya ve âhiret iyiliği',
    arabic:
      'رَبَّنَٓا اٰتِنَا فِي الدُّنْيَا حَسَنَةً وَفِي الْاٰخِرَةِ حَسَنَةً وَقِنَا عَذَابَ النَّارِ',
    transliteration:
      'Rabbenâ âtinâ fid-dünyâ haseneten ve fil-âhirati haseneten ve kınâ azâben-nâr.',
    meaning:
      'Rabbimiz! Bize dünyada da iyilik ver, âhirette de iyilik ver ve bizi ateş azabından koru.',
    reference: 'Bakara sûresi, 201. âyet',
    reviewed: true,
  },
  {
    id: 'bakara-250',
    categoryId: 'kuran',
    title: 'Sabır ve sebat',
    arabic:
      'رَبَّنَٓا اَفْرِغْ عَلَيْنَا صَبْرًا وَثَبِّتْ اَقْدَامَنَا وَانْصُرْنَا عَلَى الْقَوْمِ الْكَافِرِينَ',
    transliteration:
      'Rabbenâ efriğ aleynâ sabran ve sebbit akdâmenâ vansurnâ alel-kavmil-kâfirîn.',
    meaning:
      'Rabbimiz! Üzerimize sabır yağdır, ayaklarımızı sağlam tut ve inkârcı topluluğa karşı bize yardım et.',
    reference: 'Bakara sûresi, 250. âyet',
    reviewed: true,
  },
  {
    id: 'bakara-286',
    categoryId: 'kuran',
    title: 'Gücümüzün üstünde yük yükleme',
    arabic:
      'رَبَّنَا لَا تُؤَاخِذْنَٓا اِنْ نَسِينَٓا اَوْ اَخْطَاْنَا، رَبَّنَا وَلَا تَحْمِلْ عَلَيْنَٓا اِصْرًا كَمَا حَمَلْتَهُ عَلَى الَّذِينَ مِنْ قَبْلِنَا، رَبَّنَا وَلَا تُحَمِّلْنَا مَا لَا طَاقَةَ لَنَا بِهِ، وَاعْفُ عَنَّا وَاغْفِرْ لَنَا وَارْحَمْنَا',
    transliteration:
      'Rabbenâ lâ tüâhıznâ in nesînâ ev ahta’nâ. Rabbenâ ve lâ tahmil aleynâ ısran kemâ hameltehû alellezîne min kablinâ. Rabbenâ ve lâ tühammilnâ mâ lâ tâkate lenâ bih. Va’fü annâ vağfir lenâ verhamnâ.',
    meaning:
      'Rabbimiz! Unutur veya yanılırsak bizi sorumlu tutma. Rabbimiz! Bizden öncekilere yüklediğin gibi bize de ağır yük yükleme. Rabbimiz! Gücümüzün yetmeyeceği şeyi bize taşıtma. Bizi affet, bağışla ve bize merhamet et.',
    reference: 'Bakara sûresi, 286. âyet',
    reviewed: true,
  },
  {
    id: 'ali-imran-8',
    categoryId: 'kuran',
    title: 'Kalbi eğriltme',
    arabic:
      'رَبَّنَا لَا تُزِغْ قُلُوبَنَا بَعْدَ اِذْ هَدَيْتَنَا وَهَبْ لَنَا مِنْ لَدُنْكَ رَحْمَةً، اِنَّكَ اَنْتَ الْوَهَّابُ',
    transliteration:
      'Rabbenâ lâ tüziğ kulûbenâ ba’de iz hedeytenâ veheb lenâ min ledünke rahmeh. İnneke entel-vehhâb.',
    meaning:
      'Rabbimiz! Bizi doğru yola ilettikten sonra kalplerimizi eğriltme ve katından bize rahmet bağışla. Şüphesiz sen çok bağışlayansın.',
    reference: 'Âl-i İmrân sûresi, 8. âyet',
    reviewed: true,
  },
  {
    id: 'ali-imran-147',
    categoryId: 'kuran',
    title: 'Günahların bağışlanması',
    arabic:
      'رَبَّنَا اغْفِرْ لَنَا ذُنُوبَنَا وَاِسْرَافَنَا فِٓي اَمْرِنَا وَثَبِّتْ اَقْدَامَنَا وَانْصُرْنَا عَلَى الْقَوْمِ الْكَافِرِينَ',
    transliteration:
      'Rabbenâğfir lenâ zünûbenâ ve isrâfenâ fî emrinâ ve sebbit akdâmenâ vansurnâ alel-kavmil-kâfirîn.',
    meaning:
      'Rabbimiz! Günahlarımızı ve işimizdeki aşırılıklarımızı bağışla, ayaklarımızı sağlam tut ve inkârcı topluluğa karşı bize yardım et.',
    reference: 'Âl-i İmrân sûresi, 147. âyet',
    reviewed: true,
  },
  {
    id: 'ali-imran-193',
    categoryId: 'kuran',
    title: 'İyilerle birlikte olmak',
    arabic:
      'رَبَّنَا فَاغْفِرْ لَنَا ذُنُوبَنَا وَكَفِّرْ عَنَّا سَيِّـَٔاتِنَا وَتَوَفَّنَا مَعَ الْاَبْرَارِ',
    transliteration:
      'Rabbenâ fağfir lenâ zünûbenâ ve keffir annâ seyyiâtinâ ve teveffenâ meal-ebrâr.',
    meaning:
      'Rabbimiz! Günahlarımızı bağışla, kötülüklerimizi ört ve canımızı iyilerle beraber al.',
    reference: 'Âl-i İmrân sûresi, 193. âyet',
    reviewed: true,
  },
  {
    id: 'araf-23',
    categoryId: 'kuran',
    title: 'Tövbe — Âdem duası',
    arabic:
      'رَبَّنَا ظَلَمْنَٓا اَنْفُسَنَا وَاِنْ لَمْ تَغْفِرْ لَنَا وَتَرْحَمْنَا لَنَكُونَنَّ مِنَ الْخَاسِرِينَ',
    transliteration:
      'Rabbenâ zalemnâ enfüsenâ ve in lem tağfir lenâ ve terhamnâ lenekûnenne minel-hâsirîn.',
    meaning:
      'Rabbimiz! Kendimize yazık ettik. Bizi bağışlamaz ve bize merhamet etmezsen mutlaka kaybedenlerden oluruz.',
    reference: 'A’râf sûresi, 23. âyet',
    reviewed: true,
  },
  {
    id: 'taha-25-28',
    categoryId: 'kuran',
    title: 'Göğsü genişletme — Mûsâ duası',
    when: 'Zor bir işe başlarken, konuşma ve anlatma öncesinde.',
    arabic:
      'رَبِّ اشْرَحْ لِي صَدْرِي، وَيَسِّرْ لِٓي اَمْرِي، وَاحْلُلْ عُقْدَةً مِنْ لِسَانِي، يَفْقَهُوا قَوْلِي',
    transliteration:
      'Rabbişrah lî sadrî, ve yessir lî emrî, vahlül ukdeten min lisânî, yefkahû kavlî.',
    meaning:
      'Rabbim! Göğsümü genişlet, işimi kolaylaştır, dilimdeki düğümü çöz ki sözümü anlasınlar.',
    reference: 'Tâhâ sûresi, 25-28. âyetler',
    reviewed: true,
  },
  {
    id: 'taha-114',
    categoryId: 'kuran',
    title: 'İlim artışı',
    when: 'Ders, okuma ve öğrenme öncesinde.',
    arabic: 'رَبِّ زِدْنِي عِلْمًا',
    transliteration: 'Rabbi zidnî ilmâ.',
    meaning: 'Rabbim! İlmimi artır.',
    reference: 'Tâhâ sûresi, 114. âyet',
    reviewed: true,
  },
  {
    id: 'furkan-74',
    categoryId: 'kuran',
    title: 'Aile ve nesil için',
    arabic:
      'رَبَّنَا هَبْ لَنَا مِنْ اَزْوَاجِنَا وَذُرِّيَّاتِنَا قُرَّةَ اَعْيُنٍ وَاجْعَلْنَا لِلْمُتَّقِينَ اِمَامًا',
    transliteration:
      'Rabbenâ heb lenâ min ezvâcinâ ve zürriyyâtinâ kurrate a’yünin vec’alnâ lil-müttakîne imâmâ.',
    meaning:
      'Rabbimiz! Eşlerimizi ve çocuklarımızı bize göz aydınlığı kıl ve bizi takvâ sahiplerine önder yap.',
    reference: 'Furkân sûresi, 74. âyet',
    reviewed: true,
  },
  {
    id: 'ibrahim-40',
    categoryId: 'kuran',
    title: 'Namazı ayakta tutmak',
    arabic:
      'رَبِّ اجْعَلْنِي مُقِيمَ الصَّلٰوةِ وَمِنْ ذُرِّيَّتِي، رَبَّنَا وَتَقَبَّلْ دُعَٓاءِ',
    transliteration:
      'Rabbic’alnî mükîmes-salâti ve min zürriyyetî. Rabbenâ ve tekabbel duâ.',
    meaning:
      'Rabbim! Beni ve soyumdan gelenleri namazı gereğince kılanlardan eyle. Rabbimiz! Duamı kabul buyur.',
    reference: 'İbrâhim sûresi, 40. âyet',
    reviewed: true,
  },
  {
    id: 'hasr-10',
    categoryId: 'kuran',
    title: 'Müminler için bağışlanma',
    arabic:
      'رَبَّنَا اغْفِرْ لَنَا وَلِاِخْوَانِنَا الَّذِينَ سَبَقُونَا بِالْاِيمَانِ وَلَا تَجْعَلْ فِي قُلُوبِنَا غِلًّا لِلَّذِينَ اٰمَنُوا',
    transliteration:
      'Rabbenâğfir lenâ ve li-ihvâninellezîne sebekûnâ bil-îmâni ve lâ tec’al fî kulûbinâ ğillen lillezîne âmenû.',
    meaning:
      'Rabbimiz! Bizi ve bizden önce iman etmiş kardeşlerimizi bağışla; kalplerimizde iman edenlere karşı kin bırakma.',
    reference: 'Haşr sûresi, 10. âyet',
    reviewed: true,
  },
];
